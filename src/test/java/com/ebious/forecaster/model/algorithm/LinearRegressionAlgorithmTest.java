...
class LinearRegressionAlgorithmTest {
    ...
    final ForecastAlgorithm linear = new LinearRegressionAlgorithm();
    ...
    void forecastOnADate() {
        ...
        Rate rate = linear.forecastOnADate(dataSet.get(currency), expectedDate);
        SoftAssertions softAssertions = new SoftAssertions();
        softAssertions.assertThat(rate.getDate()).isEqualTo(expectedDate);
        softAssertions.assertThat(rate.getCurs()).isBetween(100D, 110D);
    ...
    void forecastToPeriod() {
        ...
        Currency currency = Currency.USD;
        Map<Currency, List<Rate>> dataSet = supplier.deliverDataSet(Arrays.asList(currency));
        List<Rate> rates = linear.forecastToPeriod(dataSet.get(currency), period);
        assertSoftly(
                softAssertions -> {
                    for (int i = 0, daysCount = 1; i < rates.size(); i++, daysCount++) {
                        ...
                        softAssertions.assertThat(rates.get(i).getCurs()).isBetween(99D, 110D);
                    }
                }
        );
    }
